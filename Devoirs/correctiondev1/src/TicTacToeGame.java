...
// FINISH THE VARIABLE DECLARATION
	...
	* pointe vers un tableau d’objets de type 
	* CellValue afin de représenter l’état de la grille.
	*/
	CellValue[] board;
	//zak here, it's a one dim array
	//so creating it is a matter of multiplying the numb of lines and columns
	//aka the "size"
    ...
    *  the number of columns in the game
	...
	public TicTacToeGame(int lines, int columns){
		...
		// YOUR CODE HERE why are ther emultiple constuctos
    ...
    *  the number of cells that must be aligned to win.
  	*/
	public TicTacToeGame(int lines=3, int columns=3, int sizeWin=3){
		...
		//check if win 
			for(int i=1; i<sizeWin; i++){
				//Diagonals
				boolean diagX = False;
				if(board[i][i]==CellValue.X && board[i+1][i+1]==CellValue.X && board[i+1][i+1]!=CellValue.EMPTY && board[i][i]!=CellValue.EMPTY){
					;
				}//close if
			}
		}//close while
